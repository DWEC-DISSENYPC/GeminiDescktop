// ==========================================================================
// Renderer Process Logic (Gemini Desktop Shell)
// ==========================================================================
...
function loadProfileTabs() {
  ...
  // Spawn 3 tabs by default for Gemini (all logged into the same account session)
  createTab('https://gemini.google.com');
  createTab('https://gemini.google.com');
  createTab('https://gemini.google.com');
...
function createTab(url = 'https://gemini.google.com') {
...
function setupWebviewEvents(tab) {
  ...
  // Intercept links inside webview to keep application locked to Gemini
  ...
  webview.addEventListener('new-window', (e) => {
    ...
    if (url.startsWith('https://gemini.google.com')) {
      // If it's a Gemini URL, open a new tab internally
  ...
  webview.addEventListener('page-title-updated', (e) => {
    ...
    // Clean Gemini title templates
    if (cleanTitle.endsWith(' - Gemini')) {
      cleanTitle = cleanTitle.substring(0, cleanTitle.length - 9);
    }
    if (cleanTitle.toLowerCase() === 'gemini') {
      cleanTitle = 'Conversación';
    }
...
function selectTab(tabId) {
  ...
  if (newActiveTab) {
    ...
    if (newActiveTab.isLoading) {
      ...
      DOM.reloadSvg.classList.add('hidden');
      DOM.stopSvg.classList.remove('hidden');
      DOM.addressInput.textContent = 'https://gemini.google.com';
    } else {
      DOM.appLoadingScreen.classList.add('hidden');
      DOM.reloadSvg.classList.remove('hidden');
      DOM.stopSvg.classList.add('hidden');
...
function closeTab(tabId) {
  ...
  if (state.tabs.length <= 1) {
    // Just reload it back to Gemini home if it's the only one
